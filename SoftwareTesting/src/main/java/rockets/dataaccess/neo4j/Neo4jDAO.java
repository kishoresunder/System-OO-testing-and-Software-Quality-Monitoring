...
public class Neo4jDAO implements DAO {
	...
	public <T extends Entity> T createOrUpdate(T entity) {
		...
		saveOutgoingEntities(entity, clazz, existingEntity);
		session.save(entity);
		tx.commit();
		return entity;
	}
	...
	private <T extends Entity> void saveOutgoingEntities(T entity, Class clazz, T existingEntity) {
		if (clazz.equals(Rocket.class)) {
			Rocket rocket = (Rocket) entity;
			//createOrUpdate(rocket);
			if (null != rocket.getLaunches()) {
