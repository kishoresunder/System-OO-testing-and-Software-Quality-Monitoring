...
//        handleGetLaunches();
		...
		post("/register", (req, res) -> {
			...
			try {
				...
				if (userCollection == null || userCollection.isEmpty()) {
					System.out.println("can create 1");
					dao.createOrUpdate(user);
				} else {
					System.out.println("cannot create 2");
					for (User userVal : userCollection) {
						if (email.equalsIgnoreCase(userVal.getEmail())) {
							//System.out.println("is present 3");
							isPresent = true;
						}else{
				...
				if(isPresent) {
					System.out.println("inside login 4");
					res.redirect("/login");
				}else{
					res.redirect("/hello");
				}
	...
	private static Rocket getLoggedInRocket(Request req) {
		spark.Session session = req.session();
		Rocket rocket = null;
		if (null != session) {
	...
	private static void handleGetRocket() {
		get("/rocket/:id", (req, res) -> {
			Map<String, Object> attributes = new HashMap<>();
			Rocket rocket = getLoggedInRocket(req);
			attributes.put("rocket", rocket);
			try {
				String id = req.params(":id");
					Rocket roc = dao.load(Rocket.class, Long.parseLong(id));
				if (null != roc) {
					attributes.put("rocket", roc);
				} else {
					attributes.put("errorMsg", "No Rocket with the name " + id + ".");
				}
	...
	// TODO: Need to TDD this // display the creation page
	...
	private static void handleGetRockets() {
		get("/rockets", (req, res) -> {
			Map<String, Object> attributes = new HashMap<>();
			try {
				Collection<Rocket> rocket=dao.loadAll(Rocket.class);
